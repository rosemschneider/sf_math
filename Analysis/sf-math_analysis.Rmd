...
sf.raw <- read.csv("Data/SF.csv")%>%
  rename(num_queried = starting_num) #to make life easier for within/outside
...
```
Change the age for one participant with incorrect age
```{r}
sf.df %<>%
...
ggplot(sf.prod.ms, aes(x = productive, y = mean, fill = productive)) + 
...
ggplot(sf.range.ms, aes(x = productive, y = mean, fill = count_range)) + 
  geom_bar(stat = "identity", position= position_dodge()) + 
  geom_linerange(aes(ymin = ci_lower,
                      ymax = ci_upper),
                  size = .3,
...
#Meaningful way of classifying
#Visualize
nn.prod.ms <- nextnumber.df %>%
  group_by(productive)%>%
  multi_boot_standard("correct", na.rm= TRUE)

ggplot(nn.prod.ms, aes(x = productive, y = mean, fill = productive)) + 
  geom_bar(stat = "identity", position= position_dodge()) + 
  geom_linerange(aes(ymin = ci_lower,
                      ymax = ci_upper),
                  size = .3,
...
ggplot(nn.range.ms, aes(x = productive, y = mean, fill = count_range)) + 
  geom_bar(stat = "identity", position= position_dodge()) + 
  geom_linerange(aes(ymin = ci_lower,
                      ymax = ci_upper),
                  size = .3,
