...
check <- unique.given %>%
  filter(SID %!in% unique.mathfacts$SID)
...
# }
# 
# test <- class_prod(unique_SIDs)%>%
#   rename(check_prod = productive)%>%
#   mutate(check_prod = ifelse(check_prod == TRUE, "productive", "nonproductive"))
# 
# #check auto to manual coding
# hc.prod <- highestcount.df %>%
#   distinct(SID, productive)
# 
# validate2 <- right_join(hc.prod, test, by = "SID")%>%
#   mutate(check = ifelse(productive == check_prod, 1, 0))
# 
...
###Model 7 Results
Math Facts does not significantly improve the model fit (Chisq(6) = `r round(anova(model.7.base, model.7.mf, test = 'LRT')$Chisq[2], 2)`, p = `r round(anova(model.7.base, model.7.mf, test = 'LRT')$Pr[2], 2)`.) We have about half of our data currently, so we will see what happens here!
...
three_box <- ggplot(three_limited, aes(x = task, y = correct, fill = 
