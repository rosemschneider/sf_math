...
Which measure of productivity best fits the 
```{r}
...
##Do children perform worse on every problem between NN, Unit, and MF?
...
sf.mf.comparison %>%
  mutate(Task = factor(Task, levels = c("SF", "MF"), labels = c("Unit", "Math Facts")),
         magnitude.match = sort(magnitude.match),
         magnitude.match = factor(magnitude.match))%>%
...
No; there is no interaction. Children are just uniformly worse on the MF task in comparison to SF. 
```{r}
#build a base model
comp.sf.mf.base <- glmer(Correct ~ Task + count_range + age.c + (1|SID), 
                data =  sf.mf.comparison, 
                family = "binomial", 
...
#compare
anova(comp.sf.mf.base, comp.sf.mf.mag,comp.sf.mf.int, test = 'lrt')
```
...
```{r}
matched.items <- c(5, 6, 20, 32, 57, 62, 64, 86, 84, 93)

match.df <- sf.mf.comparison %>%
  filter(Task_item %in% matched.items)%>%
  dplyr::select(SID, sf.quartile, Task, magnitude.match, Correct)%>%
  spread(key = "Task", value = "Correct")%>%
   mutate(magnitude.match = sort(magnitude.match), 
         magnitude.match = factor(magnitude.match))%>%
  group_by(SID, sf.quartile, magnitude.match)%>%
  mutate(sf.mf.success = ifelse((MF == 1 & SF == 1), "SF and MF correct", 
                                ifelse((MF == 0 & SF == 1), "MF incorrect", 
                                ifelse((MF == 1 & SF ==0), "SF incorrect", "SF and MF incorrect"))))

match.df %>% 
  filter(!is.na(sf.mf.success))%>%
  group_by(sf.quartile, magnitude.match, sf.mf.success)%>%
  summarise(n = n())%>%
  ggplot(aes(x = magnitude.match, y= n, fill = sf.mf.success)) + 
  geom_bar(stat = "identity") +
  facet_grid(~sf.mf.success) + 
  theme(legend.position = "top")

#For a given magnitude, what is the proportion of children who succeed on SF, but fail on MF? 
match.df %>%
  filter(!is.na(sf.mf.success))%>%
  group_by(magnitude.match, sf.mf.success)%>%
  summarise(n = n())%>%
  group_by(magnitude.match)%>%
  mutate(total.n = sum(n), 
         prop = n/total.n)%>%
  ggplot(aes(x = magnitude.match, y = prop, fill = sf.mf.success)) + 
  geom_bar(stat = "identity")

#roughly 50% of children succeed on SF but fail on MF for every number

#what about by unit task performance? 
match.df %>%
  filter(!is.na(sf.mf.success))%>%
  group_by(sf.quartile, magnitude.match, sf.mf.success)%>%
  summarise(n = n())%>%
...
#is there a difference in the number of children who succeed on SF but fail on MF for a given number across quartiles? 


#### !! what I want to do is: 
#identify kids at equation 'understanders' based on spontaneous response to 1+1 - this will require a bit of re-coding, I think
#then, looking at children who understand 1+1, separate Unit task problems into success and failure
#then, for a given magnitude, what proportion of children either succeeded or failed for mf?
...
understander.df %>%
  filter(Task_item %in% matched.items,
         equation.understander == "understander")%>%
  filter(!is.na(equation.understander))%>%
  dplyr::select(SID, Task, magnitude.match, equation.understander, Correct)%>%
  spread(key = "Task", value = "Correct")%>%
  mutate(magnitude.match = factor(sort(magnitude.match)))%>%
  mutate(sf.success = ifelse(SF == 1, "sf correct", "sf incorrect"), 
         mf.success = ifelse(MF == 1, "mf correct", "mf incorrect"), 
         mf.success = factor(mf.success, levels = c("mf incorrect", "mf correct"), 
                             labels = c("Incorrect Math Facts", "Correct Math Facts")), 
         sf.success = factor(sf.success, levels = c("sf correct", "sf incorrect"), 
                             labels = c("Correct Unit Task items", "Incorrect Unit Task Items")))%>%
  group_by(magnitude.match, equation.understander, sf.success, mf.success)%>%
  summarise(n = n())%>%
  group_by(magnitude.match)%>%
  mutate(total.n = sum(n), 
         prop = n/total.n)%>%
  ggplot(aes(x = magnitude.match, y = prop, fill =  mf.success)) +
  geom_bar(stat = "identity") + 
  facet_grid(~sf.success) + 
  labs(y = 'Proportion of children', x = "Magnitude of MF problem") + 
  scale_fill_brewer(palette = "Set1")

#what about for the same items in both tasks
understander.df %>%
  filter(Task_item == 20 | 
           Task_item == 32 | 
           Task_item == 57 | 
           Task_item == 93)%>%
  filter(!is.na(equation.understander))%>%
  dplyr::select(SID, Task, Task_item, equation.understander, Correct)%>%
  spread(key = "Task", value = "Correct")%>%
  mutate(Task_item = factor(sort(Task_item)))%>%
  mutate(sf.success = ifelse(SF == 1, "sf correct", "sf incorrect"), 
         mf.success = ifelse(MF == 1, "mf correct", "mf incorrect"), 
         mf.success = factor(mf.success, levels = c("mf incorrect", "mf correct"), 
                             labels = c("Incorrect Math Facts", "Correct Math Facts")), 
         sf.success = factor(sf.success, levels = c("sf correct", "sf incorrect"), 
                             labels = c("Correct Unit Task items", "Incorrect Unit Task Items")), 
         equation.understander = factor(equation.understander, levels = c("understander", "non-understander"), 
                                        labels = c("Addition knower (5+1)", "Addition non-knower (5+1)")))%>%
  group_by(Task_item, equation.understander, sf.success, mf.success)%>%
  filter(!is.na(mf.success))%>%
  summarise(n = n())%>%
  group_by(Task_item)%>%
  mutate(total.n = sum(n), 
         prop = n/total.n)%>%
  ggplot(aes(x = Task_item, y = n, fill =  mf.success)) +
  geom_bar(stat = "identity", position = position_dodge(preserve = 'single'), color = "black") + 
  facet_grid(equation.understander~sf.success, scale = "free_x") + 
  theme_bw(base_size = 10) +
  labs(y = 'Number of children correct/incorrect', x = "Math Facts Problem") + 
  scale_fill_brewer(palette = "Set1") + 
  guides(fill = guide_legend(title = "Math Facts performance")) + 
  theme(panel.grid = element_blank())

  
#what is the average MF performance of kids who "get" MF for these items? 
understander.ms <- understander.df %>%
  filter(Task_item == 20 | 
           Task_item == 32 | 
           Task_item == 57 | 
           Task_item == 93)%>%
...
```{r}
#For all children, test whether there is a difference between tasks for items that were repeated across both tasks

#Correct ~ Number*Task + count_range + age.c + (1|SID)

trial.type.contrasts <- rbind(c(1, -0.5, -0.5),     # MF vs. (SF + NN)/2
             c(0, 1, -1))      # sf vs. NN        
cMat <- MASS::ginv(trial.type.contrasts)

planned.contrasts <- glmer(Correct ~ Task + count_range + age.c + (1|SID), 
                           data = subset(trial.type.df, trial.type == "SF_MF_same" | trial.type == "SF_NN_same"), 
             family = "binomial",
            contrasts = list(Task = cMat), 
            control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
...
##interim: for items that are repeated, MF is worse than SF by a lot; SF is worse than NN by a little

#look at magnitude match*task interaction
task.contrasts <- rbind(c(1, -0.5, -0.5),     # MF vs. (SF + NN)/2
             c(0, 1, -1))      # sf vs. NN        
cMat <- MASS::ginv(task.contrasts)

planned.contrasts <- glmer(Correct ~ magnitude.match.c*Task + count_range + age.c + (1|SID), 
                           data = trial.type.df, 
             family = "binomial",
            contrasts = list(Task = cMat), 
            control=glmerControl(optimizer="bobyqa",
...
#for all kids
trial.type.df %>%
  mutate(Task = factor(Task, levels = c("SF", "WCN", "MF"), labels = c("Unit", 
                                                                       "Next Number", 
                                                                       "Math Facts")),
         magnitude.match = sort(magnitude.match),
         magnitude.match = factor(magnitude.match))%>%
...
trial.type.df %>%
  mutate(Task_item = factor(Task_item, levels = c("5", "6", "7", "15", "20", 
         ...
         Task = factor(Task, levels = c("SF", "WCN", "MF"), labels = c("Unit", 
                                                                       "Next Number", 
                                                                       "Math Facts")))%>%
  group_by(trial.type, Task, Task_item)%>%
  summarise(mean = mean(Correct, na.rm = TRUE), 
            n = n(), 
...
##some sanity
#what I want to know: 
#for items that are repeated across both tasks
#does performance differ between MF and SF and NN and SF
```
