...
sf.df <- add_highest_num(nextnumber.df)
```
...
#Model 1: Highest next number (p < .0001)
#Model 2: Initial highest (p = .07)
#Model 3: Final highest (p < .0001)
#Model 4: Productive (p = .0002)

model.5.base <- glmer(correct ~ count_range + age + (1|SID), 
                      family = "binomial", data = model.df)
#add nn, compare
model.5.highest_nn <- glmer(correct ~ count_range + age + highest_num + (1|SID), 
                            family = "binomial", data = model.df)

anova(model.5.base, model.5.highest_nn, test = 'LRT') #significant, retain

#add final highest
model.5.final_highest <- glmer(correct ~ count_range + age + highest_num +
                                 final_highest + (1|SID), family = "binomial", 
                               data = model.df)
...
#compare
anova(model.5.highest_nn, model.5.final_highest, test = 'LRT') #significant, retain both

##add productive
model.5.productive <- glmer(correct ~ count_range + age + highest_num + 
                              final_highest + productive + (1|SID), family = "binomial", 
                            data = model.df)

#model failed to converge - check to see if this is a real issue
with(model.5.productive@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3) #False, not converged -- this is most likely an issue because we don't have enough data at this point
...
4. Error classification on highest count
...
##performance on individ items
# mf.items <- mathfacts.df %>%
#   filter(problem != "1+1")%>%
#   group_by(problem, count_range, productive)%>%
#   multi_boot_standard("correct", na.rm = TRUE)
# 
# ggplot(mf.items, aes(x = problem, y = mean, 
#                         fill = factor(count_range, levels = c("within", "outside"), 
#                                       labels = c("Within initial highest count", "Outside initial highest count")))) + 
#   geom_bar(stat = "identity", position= position_dodge()) + 
#   geom_linerange(aes(ymin = ci_lower,
...
#        title = "Accuracy by count range and productivity: Math Facts", 
#        fill = "Within/outside count range") +
#   facet_wrap(~productive, scale = "free_x") +
...
sf.nn.pcor <- pcor.test(all.mean$mean.sf, all.mean$mean.nn, all.mean$age) #this is doing weird stuff, not sure how to deal with it right now
