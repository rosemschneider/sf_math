...
make_single_model <- function(df, modelType) {
  if (modelType == "Base") { 
    model <- glmer(Correct ~ count_range + age.c + (1|SID), 
      ...
      model <- glmer(substitute(Correct ~ i + count_range + age.c + (1|SID), list(i = as.name(modelType))), 
                      family = "binomial", data = df)
  }
  return(model)
}
...
```{r}
##to-do: make this flexible for more than one parameter

#make a function to test a given predictor
test_predictor <- function(base_model, parameter) {
  reduced <- base_model
  if (length(parameter) == 1) {
    full <- glmer(substitute(Correct ~ i + highest_contig.c + count_range + age.c + (1|SID), 
                             list(i = as.name(parameter))), 
                        family = "binomial", data = model.df)
  } else {
    params <- str_flatten(unlist(parameter), collapse = "+")
    full <- glmer(substitute(Correct ~ i + highest_contig.c + count_range + age.c + (1|SID), 
                             list(i = as.name(params))), 
                        family = "binomial", data = model.df)
  }
  lrt <- anova(reduced, full, test = 'LRT')
  
  if (lrt$`Pr(>Chisq)`[2] < .05) {
    print(paste(parameter, "significantly improves fit, update base model"))
    return(full)
    # return(lrt)
  }
  else {
    print(paste(parameter, "does NOT improve model, do NOT update base model"))
    return(full)
    # return(lrt)
  }
}
...
```{r}
sf.large.fhc <- test_predictor(sf.large.base, "fhc.c")
...
It does not: Chisq(1) = 
```{r}
sf.large.plusihc <- test_predictor(sf.large.fhc, "ihc.c")
...
## to add - analysis of task * problem interaction
look at the same problems from unit and NN, unit and MF
group problems by magnitude, analyze there
