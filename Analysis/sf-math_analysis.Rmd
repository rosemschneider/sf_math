...
```
In comparison to base model, there is a significant effect of Highest Next Number (Chisq.(5) = 5.2922, p = .02).
...
anova(base, model.2)
```
Interestingly, we do not find a signficant effect of Initial Highest Count. In comparison to base model, adding IHC does not significantly improve the fit of the model (Chisq(5) = .22, p = .64).
...
```
Final highest count is a significant predictor of Successor performance. In comparison to base model, the model fit is significantly improved by adding FHC (Chisq(5) = 12.96, p = .0003).
...
```
Our productive classification is a significant predictor of Successor task performance. In comparison to base model, adding Productive classification significantly improves the fit of the model (Chisq(5) = 8.00, p = .005). 
...
```
Step 1: Add FHC, compare to base model. Significantly improves model fit (Chisq(5) = 12.96, p = .0003).
...
FHC alone vs. FHC + Highest NN: Chisq(6) = .7415,p = .39
FHC alone vs. FHC + Productivity: Chisq(6) = 0, p = 1
...
```
Math Facts not a significant predictor of SF performance. Model comparison with base indicates that adding Math Facts does not significantly improve model fit (Chisq(5) = .003, p = .08).
...
```
Significant correlation between age and IHC (r = .37, p = .02), and age and FHC (r = .53, p = .0003).
...
SF and age (r = .45, p = .003)
SF and IHC (r = .48, p = .0011)
SF and FHC (r = .75, p < .0001)
SF and MF (r = .46, p = .002)
SF and NN (r = .7, p < .0001)
...
```
Significant correlations between: 
MF and age (r = .34, p = .03)
MF and IHC (r = .6, p < .0001)
MF and FHC (r = .53, p = .0003)
MF and SF (r = .46, p = .002)
MF and NN (r = .34, p = .02)
...
```
Significant correlations between: 
NN and age (r = .43, p = .003)
NN and IHC (r = .46, p = .002)
NN and FHC (r = .7, p < .0001)
NN and SF (r = .7, p < .0001)
NN and MF (r = .34, p = .02)
...
productive_tasks <- ggplot(all.productive.ms, aes(x = productive, 
  ...
  labs(x = "Productivity", y = "Mean performance", 
       fill = "")
```
###t-tests of mean performance
```{r}
#sf-nn
t.test(mean.sf$mean, mean.nn$mean, var.equal = TRUE)

#sf-mf
t.test(mean.sf$mean, mean.mf$mean, var.equal = TRUE)

#nn-mf
t.test(mean.nn$mean, mean.mf$mean, var.equal = TRUE)
```
...
sf.mean <- sf.df %>%
  filter(num_queried != 1 | num_queried != 3)%>%
  group_by(SID, age, productive)%>%
...
sf.nn.pcor <- pcor.test(all.mean$mean.sf, all.mean$mean.nn, all.mean$age, method = "spearman") 
sf.nn.pcor
sf.mf.pcor <- pcor.test(all.mean$mean.sf, all.mean$mean.mf, all.mean$age, method = "spearman")
sf.mf.pcor 
```
...
clust <- highestcount.df %>%
  select(initial_highest, final_highest)
...
#Correlation between final highest and mean SF performance
```{r}
sf.mean <- model.df %>%
  group_by(SID, final_highest)%>%
  summarise(mean_correct = mean(correct))

hist(sf.mean$mean_correct)
cor.test(sf.mean$final_highest, sf.mean$mean_correct, method = "spearman")
```

#Scatterplot of final highest and mean sf performance
```{r}
sf.hc.ms <- sf.df %>%
  group_by(SID, highest_count)%>%
  summarise(mean = mean(correct))

sf.hc.scatter <- ggplot(sf.hc.ms, aes(x = mean, y = highest_count)) +
  geom_point(stat = "identity") +
  geom_smooth(method = "lm", color = "blue") + 
  theme_bw(base_size = 15) +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  labs(x = "Mean Successor Task Performance", y = "Final Highest Count")
...
t.test(subset(indefinite.df, productive == "productive")$correct, 
       subset(indefinite.df, productive == "nonproductive")$correct, 
       var.equal =TRUE)
```