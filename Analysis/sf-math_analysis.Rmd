...
model.df %<>%
  mutate(final_highest = as.integer(final_highest), #make sure this is an integer
         ...
         count_range = factor(count_range)) #change from character
```
...
```
In comparison to base model, there is a significant effect of Highest Next Number (Chisq(5) = `r anova(base, model.1, test = 'LRT')$Chisq[2]`, p = `r anova(base, model.1, test = 'LRT')$Pr[2]`.)
...
anova(base, model.2)
```
Interestingly, we do not find a signficant effect of Initial Highest Count. In comparison to base model, adding IHC does not significantly improve the fit of the model (Chisq(5) = `r anova(base, model.2)$Chisq[2]`, p = `r anova(base, model.2)$Pr[2]`.)
...
```
Final highest count is a significant predictor of Successor performance. In comparison to base model, the model fit is significantly improved by adding FHC (Chisq(5) = `r anova(base, model.3, test = 'LRT')$Chisq[2]`, p = `r anova(base, model.3, test = 'LRT')$Pr[2]`.)
...
```
Our productive classification is a significant predictor of Successor task performance. In comparison to base model, adding Productive classification significantly improves the fit of the model (Chisq(5) = `r anova(base, model.4, test = 'LRT')$Chisq[2]`, p = `r anova(base, model.4, test = 'LRT')$Pr[2]`.)
...
```
Step 1: Add FHC, compare to base model. Significantly improves model fit (Chisq(5) = `r anova(model.5.base, model.5.final, test = 'LRT')$Chisq[2]`, p = `r anova(model.5.base, model.5.final, test = 'LRT')$Pr[2]`.)
...
FHC alone vs. FHC + Highest NN: Chisq(6) = `r anova(model.5.highest_nn,  model.5.prod,model.5.final, test = 'LRT')$Chisq[2]`, p = `r anova(model.5.highest_nn,  model.5.prod,model.5.final, test = 'LRT')$Pr[2]`.
FHC alone vs. FHC + Productivity: Chisq(6) = `r anova(model.5.highest_nn,  model.5.prod,model.5.final, test = 'LRT')$Chisq[3]`, p = `r anova(model.5.highest_nn,  model.5.prod,model.5.final, test = 'LRT')$Pr[3]`.
...
```
Math Facts *is* a significant predictor of SF performance. Model comparison with base indicates that adding Math Facts does significantly improve model fit (Chisq(5) = `r anova(model.6, model.6.base, test = 'LRT')$Chisq[2]`, p = `r anova(model.6, model.6.base, test = 'LRT')$Pr[2]`.)
...
```
Math Facts does not significantly improve the model fit, indicating that our Productivity Measure (Final Highest Count) is doing the majority of the work in predicting SF performance (Chisq(6) = `r anova(model.7.base, model.7.mf, test = 'LRT')$Chisq[2]`, p = `r anova(model.7.base, model.7.mf, test = 'LRT')$Pr[2]`.)

###Exploratory model: Highest contiguous NN
We did not account for accuracy in highest NN. Thus, it is possible for a child to answer a very high NN by chance, which would add some noise to this measure. Below, I'm pulling out the 
...
Age and IHC: r = `r as.numeric(cor.test(cor.mean$age, cor.mean$initial_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$age, cor.mean$initial_highest)$p.value)`
Age and FHC: r = `r as.numeric(cor.test(cor.mean$age, cor.mean$final_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$age, cor.mean$final_highest)$p.value)`
...
SF and age: r = `as.numeric(cor.test(cor.mean$mean.sf, cor.mean$age)$estimate)`, p = `as.numeric(r cor.test(cor.mean$mean.sf, cor.mean$age)$p.value)`
SF and IHC: r = `as.numeric(cor.test(cor.mean$mean.sf, cor.mean$initial_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.sf, cor.mean$initial_highest)$p.value)`
SF and FHC: r = `as.numeric(cor.test(cor.mean$mean.sf, cor.mean$final_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.sf, cor.mean$final_highest)$p.value)`
SF and MF: r = `as.numeric(cor.test(cor.mean$mean.sf, cor.mean$mean.mf)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.sf, cor.mean$mean.mf)$p.value)`
SF and NN:  r = `as.numeric(cor.test(cor.mean$mean.nn, cor.mean$mean.mf)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.sf, cor.mean$mean.nn)$p.value)`
...
```
Significant correlations between: 
MF and age: r = `as.numeric(cor.test(cor.mean$mean.mf, cor.mean$age)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.mf, cor.mean$age)$p.value)`
MF and IHC: r = `as.numeric(cor.test(cor.mean$mean.mf, cor.mean$initial_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.mf, cor.mean$initial_highest)$p.value)`
MF and FHC: r = `as.numeric(cor.test(cor.mean$mean.mf, cor.mean$final_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.mf, cor.mean$final_highest)$p.value)`
MF and SF: r = `as.numeric(cor.test(cor.mean$mean.mf, cor.mean$mean.sf)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.mf, cor.mean$mean.sf)$p.value)`
MF and NN: r = `as.numeric(cor.test(cor.mean$mean.mf, cor.mean$mean.nn)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.mf, cor.mean$mean.nn)$p.value)`
...
```
Significant correlations between: 
NN and age: r = `as.numeric(cor.test(cor.mean$mean.nn, cor.mean$age)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.nn, cor.mean$age)$p.value)`
NN and IHC: r = `as.numeric(cor.test(cor.mean$mean.nn, cor.mean$initial_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.nn, cor.mean$initial_highest)$p.value)`
NN and FHC: r = `as.numeric(cor.test(cor.mean$mean.nn, cor.mean$final_highest)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.nn, cor.mean$final_highest)$p.value)`
NN and SF: r = `as.numeric(cor.test(cor.mean$mean.nn, cor.mean$mean.sf)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.nn, cor.mean$mean.sf)$p.value)`
NN and MF: r = `as.numeric(cor.test(cor.mean$mean.nn, cor.mean$mean.mf)$estimate)`, p = `r as.numeric(cor.test(cor.mean$mean.nn, cor.mean$mean.mf)$p.value)`
...
partial
```
Significant correlation between mean performance on SF and Indefinite tasks when controlling for age (r = `r as.numeric(partial$estimate)`, p = `r as.numeric(partial$p.value)`). 
...
Next Number: t(51) = `r as.numeric(nn.t.test$statistic)`, p = `r as.numeric(nn.t.test$p.value)`
Successor: t(51) = `r as.numeric(sf.t.test$statistic)`, p = `r as.numeric(sf.t.test$p.value)`
Math Facts: t(51) = `r as.numeric(mf.t.test$statistic)`, p = `r as.numeric(mf.t.test$p.value)`
...
##Scatterplot of FHC and SF and MF and SF 
```{r}
#sf and FHC
sf.hc.ms <- sf.df %>%
...
sf.hc.scatter <- ggplot(sf.hc.ms, aes(x = highest_count, y = mean)) +
  geom_point(stat = "identity") +
  geom_smooth(method = "lm", color = "blue") + 
  theme_bw(base_size = 15) +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank()) +
  labs(x = "Final Highest Count", y = "Mean Successor Performance")
sf.hc.scatter
...
#Post-hoc analyses
1. Accuracy ~ Problem * Task: Is performance mediated by task?
```{r}
...
summary(model.post1)
```
Currently, it does not appear that the interaction of task and problem type (Novel/Same) is significant (Chisq(9) = `r anova(model.post1, model.post.base, test = 'LRT')$Chisq[2]`, p = `r anova(model.post1, model.post.base, test = 'LRT')$Pr[2]`.)

At the moment, it appears that we have a main effect of task, such that performance on Math Facts is significantly less likely to be accurate than Successor task (Beta = `r summary(model.post1)$coeff[3]`, p = `summary(model.post1)$coeff[4,3]`). There are also significant effects of age and count range.
...
ggplot(indef, aes(x = productive, y = mean, fill= productive)) + 
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_linerange(aes(ymin = ci_lower,
                      ymax = ci_upper),
                  size = .3,
                  show.legend = FALSE,
                 position=position_dodge(width = 0.9)) +
  labs(x = "Productivity", y = "Mean indefinite performance")
...
```
Productivity is a significant predictor of Successor Task performance (Chisq(4) = `r anova(base, model.1.indef, test = 'LRT')$Chisq[2]`, p = `anova(base, model.1.indef, test = 'LRT')$Pr`).