...
trial.type.df <- all.tasks.model %>%
  ...
  mutate(Task_item.factor = factor(Task_item))

#For all children, is there an interaction between numerosity and Task on SF and NN
model1 <- glmer(Correct ~ Task_item.factor*Task + count_range + age.c + (1|SID), 
                data = subset(trial.type.df, trial.type == "SF_MF_same"), 
                family = "binomial", 
                control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
#Correct ~ Number*Task + count_range + age.c + (1|SID)

number.contrasts <- rbind(c(1.2, -0.4, -0.4, -0.4),     # 20 vs. (32 + 57 + 93 ) / 3
             c(0, 1, -0.5, 0.5), # 32 vs. (57 + 93) /2
             c(0, 0, 1, -1))      # 57 vs 93         
cMat <- MASS::ginv(number.contrasts)

planned.contrasts <- glmer(Correct ~ Task_item.factor*Task + count_range + age.c + (1|SID), 
                           data = subset(trial.type.df, trial.type == "SF_MF_same"), 
             family = "binomial",
            contrasts = list(Task_item.factor = cMat), 
            control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
summary(planned.contrasts)
