...
sf.df %>%
  ...
  group_by(sex)%>%
  summarise(n = n(), mean_age = mean(age), sd_age = sd(age), median_age = median(age))
```
...
5. Error classification on highest count 
...
all.novel.ms <- all.novel %>%
  group_by(novel_same, task, productive)%>%
...
#plot
ggplot(all.novel.ms, aes(x = factor(novel_same, levels = c("novel", "same"), 
                                    labels = c("Novel", "Same")), y = mean, 
                         fill = factor(task, levels = c("math_facts", "next number", "successor"), labels = c("Math Facts", "Next Number", "Successor")))) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_linerange(aes(ymin = ci_lower,
                      ymax = ci_upper),
                  size = .3,
...
```{r}
##This might have to be coded manually, because the error coding is all over the place in the datasheet
```
