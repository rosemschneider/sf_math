...
make_single_model <- function(df, modelType) {
  if (modelType == "Base") { 
    model <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), 
                   family = "binomial", data = df)
  } else {
    formula <- paste("Correct ~", modelType, "+ count_range + age.c + (1|SID) + (1|starting_num.c)")
...
mtable.sf
# write.mtable(mtable.sf, file="Figures/single_models.txt")
```

##Step 2: Now use LRTs to test whether a given productivity predictor is significantly improving the fit of the base
IHC: Chisq(1) = 24.141, *p* > .0001; AIC = 2672.5
```{r}
anova(sf.base, sf.ihc, test = 'LRT')
```

FHC: Chisq(1) = 35.381, *p* < .0001, AIC = 2661.2
```{r}
anova(sf.base, sf.fhc, test = 'LRT')
```

Productivity/Resilience: Chisq(1) = 11.405, *p* = .0007, AIC = 2685.2
```{r}
anova(sf.base, sf.productive, test = 'LRT')
```

Highest Contiguous NN: Chisq(1) = 72.058, *p* < .0001, AIC = 2624.5
```{r}
anova(sf.base, sf.hcnn, test = 'LRT')
```

Follow-up: Highest Next Number: Chisq(1) = 10.577, *p* = .001, AIC = 2686.0
```{r}
anova(sf.base, sf.hnn, test = 'LRT')
```
...
test_predictor <- function(base_model, parameters, df) {
  ...
  if (length(parameters) == 1) {
    formula <- paste("Correct ~", parameters[1], "+ highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c)")
    ...
    formula <- paste("Correct ~", mult_params, "+ highest_contig.c + count_range + age.c + (1|SID) + (1|starting_num.c)")
    full <- glmer(formula, data = model.df, family = "binomial")
  }
...
It does: Chisq(1) = 10.447, *p* = 0.001, AIC = 2616.1
```{r}
...
```{r}
#make base model
mf.sf.ceiling.base <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = subset(all.tasks.model, sf.quartile == "(0.875,1]"))
#now add task
mf.sf.ceiling.task <- glmer(Correct ~ Task + count_range + age.c + (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = subset(all.tasks.model, sf.quartile == "(0.875,1]"))
...
planned.contrasts <- glmer(Correct ~ Task + count_range + age.c + (1|SID) +
                             (1|starting_num.c), data = subset(all.tasks.model,
                                                                           sf.quartile == "(0.875,1]"), 
             family = "binomial",
...
```{r}
#make base model
mf.sf.all.base <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = all.tasks.model)
#now add task
mf.sf.all.task <- glmer(Correct ~ Task + count_range + age.c + (1|SID) + (1|starting_num.c), 
                            family = "binomial", data = all.tasks.model)
#test to see if there is a difference by task 
...
planned.contrasts <- glmer(Correct ~ Task + count_range + age.c + (1|SID) + (1|starting_num.c), data = all.tasks.model, 
             family = "binomial",
            contrasts = list(Task = cMat))
...
all.data %>%
  ...
  scale_colour_manual(values = , guide = "none") + 
...
error.freq %>%
  filter(IHC != 120)%>% #remove kids who spontaneously count to 120
...
```{r}
error.freq.decade %>%
  filter(IHC != 120)%>% #remove kids who counted to 120 on their own
...
##Analysis: Does performance on the Indef. Task predict Unit Task performance?
Yes: Chisq = 53.73, *p* < .0001
```{r}
...
Does indefinite next number predict Unit Task when controlling for FHC, MF, and HCNN?
Yes: Chisq = 15.211, *p* < .0001
```{r}
...
##Relationship between indefinite and Unit Task
```{r}
##Highest Contiguous Next Number
all.data %>%
  filter(!is.na(mean.indef))%>%
  distinct(SID, mean.sf, mean.indef)%>%
  ggplot(aes(x = mean.indef, y = mean.sf, color = "#101D57")) + 
  geom_count(stat = "sum", 
             show.legend = FALSE) + 
  geom_smooth(aes(fill = "#101D57"), alpha = .3, 
              show.legend = FALSE) + 
  scale_color_manual(values = "#101D57") + 
  scale_fill_manual(values = "#101D57") + 
  theme_bw(base_size = 8) +
  theme(panel.grid = element_blank()) +
  labs(x = 'Mean Indefinite Next Number Performance', 
       y = "Mean Unit Task performance") 

ggsave("Figures/indef_unit.png", width = 4, height = 3)
```
...
ms.task <- all.data %>%
  distinct(SID, Productive, Age, mean.sf, mean.mf, mean.nn)%>%
  gather(key = "Task", value = "mean", -SID, - Productive, -Age)%>%
  mutate(Task = ifelse(Task == "mean.sf", "SF", 
                       ifelse(Task == "mean.nn", "WCN", "MF")))%>%
  mutate(Productive = factor(Productive, levels = c("Non-Resilient", "Resilient")), 
        age.c = as.vector(scale(Age, center = TRUE, scale=TRUE)), 
        Task = factor(Task, levels = c("MF", "SF", "WCN")))

#mean performance
resil.lm <- lm(mean ~ Productive*Task + age.c, 
               data = ms.task)
...
planned.contrasts <- lm(mean ~ Productive*Task + age.c, data = ms.task, 
            contrasts = list(Task = cMat))

summary(planned.contrasts)

#exploratory with accuracy
task.model <- all.data %>%
  filter(Task == "SF" | 
           Task == "MF" | 
           Task == "WCN")%>%
...
model.mf.sf.all.df <- all.data %>%
  filter(Task == "SF" | 
           Task == "WCN" |
           Task == "MF") %>%
  mutate(Prod.tertiary = ifelse((IHC >= 95 & Productive == "Resilient"), "Resilient IHC >= 95", 
                                ifelse((IHC < 95 & Productive == "Resilient"), "Resilient IHC < 95", "Non-Resilient")), 
         age.c = as.vector(scale(as.numeric(as.character(Age)), center = TRUE, scale = TRUE)), 
         Task = factor(Task, levels = c("MF", "SF", "WCN")))
...
#look at only participants who have items within/outside their IHC for Unit
range.base <- glmer(Correct ~ count_range*Productive + age.c + (1|SID),
                    family = "binomial", data = subset(model.df, IHC <=95))

summary(range.base)

#look at only participants who have items within/outside their IHC for NN
range.base <- glmer(Correct ~ count_range*Productive + age.c + (1|SID),
                    family = "binomial", data = subset(model.mf.sf.all.df, IHC <=95 & Task == "WCN"))

summary(range.base)

#...for MF?
range.base <- glmer(Correct ~ count_range*Productive + age.c + (1|SID),
                    family = "binomial", data = subset(model.mf.sf.all.df, IHC <=95 & Task == "MF"))
...
BIC <- mclustBIC(initial_final)

#plot
mod1 <- Mclust(initial_final, x = BIC)
summary(mod1, parameters = TRUE)
plot(mod1, what = "classification")


#now combine with SIDs so that we can do analysis
prod.clust <- function(df) {
  
  all.clust <- data.frame()
 
    #get IHC and FHC for each participant
    tmp.df <- df %>%
      distinct(SID, IHC, FHC)%>%
      dplyr::select(-SID)
...
clusters <- prod.clust(all.data)%>%
  dplyr::select(SID, IHC.x, FHC.x, CLUST)%>%
...
all.data %<>%
  dplyr::select(-IHC.y, -FHC.y)%>%
  dplyr::rename(IHC = IHC.x,
                FHC = FHC.x)

#visualize
clusters %<>%
  mutate(Cluster = factor(CLUST, levels = c(2, 4, 1, 3, 5))) 

##out of curiosity, how do these clusters do on the unit task?


ggplot(clusters, aes(x = IHC, y = FHC, color =Cluster, fill = Cluster, shape = Cluster)) + 
  geom_point(size = 3, position = position_jitter()) + 
  stat_ellipse(aes(x=IHC, y=FHC,color=Cluster, group=Cluster, fill = Cluster),
               geom="polygon", level = .8, alpha = .15, 
               type = "norm") + 
  scale_x_continuous(breaks = seq(0, 120, 10)) + 
  scale_y_continuous(breaks = seq(0, 120, 10)) + 
  theme_bw(base_size = 13) + 
  ...
  labs(x = "Initial Highest Count", 
       y= "Final Highest Count")
ggsave("Figures/GMM.png", width = 7, height = 4)



all.data %>%
  mutate(CLUST = factor(CLUST, levels = c(2, 4, 1, 3, 5)))%>%
  filter(Task == "SF")%>%
  mutate(CLUST = factor(CLUST))%>%
  group_by(SID, CLUST)%>%
  summarise(mean = mean(Correct, na.rm = TRUE))%>%
  ggplot(aes(x=CLUST, y = mean, fill = CLUST, group = CLUST)) +
  # stat_summary(fun.y = mean, position = position_dodge(width = .95), 
  #                     geom="bar", alpha = .4, colour = "black") +
  geom_violin(alpha = .2) +
  geom_point(position=position_jitterdodge(jitter.width = .5,
                                           jitter.height = .1,
                                           dodge.width = .9),
             aes(colour = CLUST, group=CLUST),
             size=2,
             alpha = .9,
               show.legend=TRUE) +
  stat_summary(fun.data = mean_cl_boot, geom="linerange", 
  ...
  langcog::scale_fill_solarized("Cluster") +
  theme(panel.grid.minor = element_blank(), 
        panel.grid.major = element_blank(), 
        legend.title = element_blank(), 
        legend.position = "none") 

ggsave("Figures/Unit_by_cluster.png")



#now, predict Unit Task by cluster 
model.df.clust <- all.data %>%
   mutate(highest_contig = as.integer(highest_contig), 
         SID = factor(SID), 
         age.c = as.vector(scale(Age, center = TRUE, scale=TRUE)), #scale and center continuous variables for model fit
         fhc.c = as.vector(scale(FHC, center = TRUE, scale = TRUE)), 
         ...
         mean.indef.c = as.vector(scale(mean.indef, center = TRUE, scale=TRUE)),
         starting_num.c = as.vector(scale(as.numeric(as.character(Task_item)), center = TRUE, scale = TRUE)), 
         CLUST = factor(CLUST, levels = c(2, 4, 1, 3, 5)))%>%
  filter(Task == "SF") # we're only predicting Unit Task performance, so filter down to this

model.clust <- glmer(Correct ~ CLUST + count_range + age.c + (1|SID) + (1|starting_num.c), 
                     family = "binomial", data = model.df.clust, 
                     control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
model.clust.base <- glmer(Correct ~ count_range + age.c + (1|SID) + (1|starting_num.c), 
                     family = "binomial", data = model.df.clust, 
                     control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))

#compare 
anova(model.clust.base, model.clust, test = 'LRT')
summary(model.clust)

##make appropriate contrast
clust.contrasts <- rbind(c(1, -0.25, -0.25, -0.25, -0.25),     # 2 vs (4,3,5,1)/4
             c(0, .75, -.25, -.25, -.25),  # 4 vs (3,5,1)/3
             c(0, 0, 1, -.5, -.5), # 1 vs (3,5)/2 
             c(0, 0, 0, 1, -1))     #3 vs. 5          
cMat <- MASS::ginv(clust.contrasts)

#starting number is not going to be informative here, I think because everyone is at ceiling in the Unit Task and WCN
planned.contrasts <- glmer(Correct ~ CLUST + count_range + age.c + (1|SID) +
                             (1|starting_num.c), data = subset(model.df.clust), 
             family = "binomial",
            contrasts = list(CLUST = cMat), 
            control=glmerControl(optimizer="bobyqa",
                         optCtrl=list(maxfun=2e4)))
summary(planned.contrasts)
```
...
ggsave("Figures/3tasks_resilience.png")
```
