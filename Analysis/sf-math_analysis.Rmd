...
cor.test(as.numeric(model.df$initial_highest), as.numeric(model.df$initial_highest))
```
...
error_plot <- ggplot(errors, aes(x = error_type, y = prop, fill = factor(productive, 
...
highestcount.df %>%
  summarise(mean_initial = mean(initial_highest), 
...
highest_count <- ggplot(initial_final, aes(x = initial_highest, y = final_highest, 
  ...
  labs(x = "Initial highest count", y = "Final highest count", 
                      color = "", title = "Initial/Final Highest Counts") +
...
sf <- ggplot(sf.range.ms, aes(x = factor(count_range, levels = c("within", "outside"), 
                                      labels = c("Within initial highest count", "Outside initial highest count")), y = mean, 
                        fill = factor(productive, levels = c("productive", "nonproductive"), 
                                   labels = c("Productive", "Nonproductive")))) + 
  geom_bar(stat = "identity", position= position_dodge()) + 
  ...
  theme(legend.position = "bottom", 
        legend.text = element_text(size = 10)) +
  labs(x = "Number queried within child's highest count?", y = "Mean Correct",  
       fill = "", title = "Successor Task Performance") +
  scale_fill_manual(values = productivity.colors) 
sf
...
ggplot(all.novel.ms, aes(x = task, y = mean, fill = productive)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  geom_linerange(aes(ymin = ci_lower,
                      ymax = ci_upper),
                  size = .3,
  ...
  labs(x = "Novel/Same", y = "Mean performance")
```
