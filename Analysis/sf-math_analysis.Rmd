...
NN.FHC.AIC <- anova(model.5.highest_nn,  model.5.prod,model.5.final, test = 'LRT')$AIC[2]
Prod.FHC.AIC <- anova(model.5.highest_nn,  model.5.prod,model.5.final, test = 'LRT')$AIC[3]
```
...
```{r}
Models <- c("Base", "Highest NN", "IHC", "FHC", "Productivity", "FHC + Highest NN", "FHC + Productivity", "Math Facts", "FHC + Math Facts")
AIC <- c(base.AIC, highestNN.AIC, IHC.AIC, FHC.AIC, Productive.AIC, NN.FHC.AIC, Prod.FHC.AIC, MF.AIC, FHC.MF.AIC)
...
full <- ggplot(full.AIC, aes (x = Models, y = AIC, fill = Models)) +
  ...
  coord_cartesian(ylim=c(920, 950))
full
