...
model.5.base <- glmer(correct ~ count_range + age + (1|SID), 
                      family = "binomial", data = model.df)
#add nn, compare
model.5.highest_nn <- glmer(correct ~ highest_num + count_range + age + (1|SID), 
                 family = "binomial", data = model.df)

anova(model.5.base, model.5.highest_nn, test = 'LRT') #significant, retain

#add final highest
model.5.final_highest <- glmer(correct ~ final_highest + highest_num + count_range +
...
##add productive
model.5.productive <- glmer(correct ~ productive + final_highest + highest_num + 
                              count_range + age + (1|SID), family = "binomial", 
                            data = model.df)

#model failed to converge - check to see if this is a real issue
with(model.5.productive@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)#true, not an issue

#now compare
anova(model.5.productive, model.5.final_highest, test = 'LRT')
...
```{r}
#first, get a vector of all the numbers used in sf 
sf_nums <- sf.df %>%
...
novel.nn <- nextnumber.df %>%
  group_by(SID, age, productive, novel_same) %>%
  multi_boot_standard("correct", na.rm = TRUE)%>%
...
#plot
ggplot(all.novel, aes(x = task, y = mean, fill = task, group = task)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  facet_wrap(~novel_same)
