...
#compare to FHC
model.exp.final <- glmer(correct ~ final_highest + highest_contig + count_range + age + (1|SID), 
                         family = "binomial", data = model.df)
...
Even with highest contiguous NN, it still seems like FHC is the best predictor of Successor performance: In comparison to base model containing FHC alone, adding highest contiguous NN does not seem to improve the model fit (Chisq(6) = `r round(anova(model.exp.final, model.exp.final.base, test = 'LRT')$Chisq[2], 2)`, p = `r round(anova(model.exp.final, model.exp.final.base, test = 'LRT')$Pr[2], 2)`.)
