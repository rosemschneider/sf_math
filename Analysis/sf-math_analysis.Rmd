...
#sanity check to make sure we have the right min and max
hist(highestcount.df$initial_highest)
hist(highestcount.df$final_highest)
```

##Highest count sanity check
Is everyone's final highest count equal to or greater than their last last successful?
```{r}
hc.check <- highestcount.df %>%
  group_by(SID)%>%
  filter(last_successful == max(last_successful))%>%
  mutate(hc.check = ifelse(final_highest >= last_successful, TRUE, FALSE))%>%
  filter(hc.check == FALSE)#one issue here, fixed in raw data sheet
#two other kids who counted past 120, but that's ok
```
...
#Filter one participant out who only has data for two tasks
given.df %<>%
...
highestcount.df <- productivity_classification(highestcount.df)
...
###Model 2 Results
In comparison to Base Model, Initial Highest Count significantly improves model fit. (Chisq(5) = `r round(anova(base, model.2)$Chisq[2], 2)`, p = `r round(anova(base, model.2)$Pr[2], 2)`.)
...
#convergence warnings - is this an issue?
with(model.6@optinfo$derivs,max(abs(solve(Hessian,gradient)))<2e-3)#true, we've adequately converged
...
model.7.mf <- glmer(correct ~ final_highest + mf_mean + count_range + age + (1|SID), 
                 family = "binomial", data = model.df)
...
###Model 7 Results
Math Facts does significantly improve the model fit (Chisq(6) = `r round(anova(model.7.base, model.7.mf, test = 'LRT')$Chisq[2], 2)`, p = `r round(anova(model.7.base, model.7.mf, test = 'LRT')$Pr[2], 2)`.) 
...
full <- ggplot(full.AIC, aes (x = Models, y = AIC, fill = Models)) +
  ...
  coord_cartesian(ylim=c(1600, 1700))
full
...
#add this number to the model df
...
anova(model.exp.final, model.exp.final.base, test = 'LRT')#significantly contributes to model fit
...
#now compare to model with math facts and final highest 
...
anova(model.exp.contig.base , model.exp.highest_contig, test = 'LRT')#math facts is still a significant predictor!

#now compare to a model with highest next number to see if it is a better predictor
model.exp.final.nn <- glmer(correct ~ highest_num + highest_contig + count_range + age + (1|SID), 
                         family = "binomial", data = model.df)
model.exp.final.nn.base <- glmer(correct ~ highest_num + count_range + age + (1|SID), 
                         family = "binomial", data = model.df)

anova(model.exp.final.nn, model.exp.final.nn.base, test = 'LRT')#highest contiguous next number is a better predictor

```

###Exploratory Model Results
The addition of Highest Contiguous NN significantly improves the fit of the model (Chisq(6) = `r round(anova(model.exp.final, model.exp.final.base, test = 'LRT')$Chisq[2], 2)`, p = `r round(anova(model.exp.final, model.exp.final.base, test = 'LRT')$Pr[2], 2)`). However, when added to a model with math facts and final highest count, it does not significantly explain additional variance.
...
```
The interaction between productivity and count range is not significant - there is a main effect of productivity, and a main effect of count range.
...
Indefinite number is a significant predictor of SF performance. 
...
```
At this time, initial highest count does not significantly predict whether someone will be productive or not.
...
productivity.colors <- c('#0083B2', '#8061A6')
...
#is productivity predictive of indefinite number performance?
