...
```{r, include = FALSE}
sf_items <- c(1 ,3, 6, 51, 84, 62, 57, 20, 32, 93, 60, 46, 81, 15, 73, 34, 95, 7)

all.data %<>%
  mutate(novel_same = ifelse((Task == "WCN" | Task == "MF") & 
                               as.integer(as.character(Task_item)) %in% sf_items, "Same", 
                             ifelse((Task == "WCN" | Task == "MF") & as.integer(as.character(Task_item)) %!in% sf_items, "Novel", NA)))

all.data %<>%
  mutate(novel_same = ifelse(Task == "SF" & (Task_item == "7" | 
                                               Task_item == "62" | 
                                               Task_item == "95" | 
                                               Task_item == "20" |
                                               Task_item == "32" |
                                               Task_item == "57" | 
                                               Task_item == "93"), "Same", "Novel"), 
         novel_same = ifelse((Task == "GiveN" | Task == "Indefinite"), NA, as.character(novel_same)))
```
...
#Task descriptives and visualizations
...
##How many errors on average by productivity?
```{r}
error.freq %>%
...
####IHC/FHC scatter - by prod. gradient
```{r}
initial_final %>%
ggplot(aes(x = IHC, y = FHC, 
                          color = prod.gradient)) +
...
all.data %>%
  ...
  mutate(Task_item = factor(Task_item, levels = c("5", "6", "7", "15", "20", 
                                                  ...
                                                  "71", "73", "81", "83", "84", "86",
                                                  "93", "95")), 
         Task = factor(Task, levels = c("SF", "WCN", "MF"), labels = c("Unit Task", 
                                                                       "Next Number", 
                                                                       "Math Facts")))%>%
...
```{r}
#base
sf.base <- glmer(Correct ~ count_range + starting_num.c +age.c + (1|SID), 
...
HCNN: 2541.2
FHC: 2573
Prod. gradient: 2576.3
IHC: 2587.3
Productive: 2594.7
Highest NN: 2599.2


##Model comparisons: Large model (with FHC)
Start with FHC, strongest predictor. NB, even though HCNN is associated with the lowest AIC, we are using FHC for our planned model comparisons, and will perform post-hoc analyses with HCNN.
```{r}
#FHC
sf.large.base <- glmer(Correct ~ fhc.c + count_range + age.c + (1|SID), 
                       family = "binomial", data = model.df)
summary(sf.large.base)

#FHC + Highest NN
sf.large.plushnn <- glmer(Correct ~ highest_num.c + fhc.c + count_range + age.c + (1|SID), 
                       family = "binomial", data = model.df)
anova(sf.large.base, sf.large.plushnn, test = 'LRT') ##highest NN does not add to the model

#IHC + FHC 
sf.large.plusihc <- glmer(Correct ~ ihc.c + fhc.c + count_range + age.c + (1|SID), 
                       family = "binomial", data = model.df)
anova(sf.large.base, sf.large.plusihc, test = 'LRT') #IHC does not add to this model

##Productivity + FHC
sf.large.plusprod <- glmer(Correct ~ Productive + fhc.c + count_range + age.c + (1|SID), 
                       family = "binomial", data = model.df)
anova(sf.large.base, sf.large.plusprod, test = 'LRT') #productivity does not add to this model 


#Prod. gradient + FHC + HCNN
sf.large.plusgain <- glmer(Correct ~ prod.gradient + fhc.c + count_range + age.c + (1|SID), 
                       family = "binomial", data = model.df)
anova(sf.large.base, sf.large.plusgain, test = 'LRT') #prod. gradient does not add to this model


##model table
mtable.sf.large <- mtable('Base Model: FHC' = sf.large.base,
            'Model 1: FHC + Highest NN' = sf.large.plushnn,
            'Model 2: FHC + IHC' = sf.large.plusihc,
            'Model 3: FHC + Prod.' = sf.large.plusprod,
            'Model 4: FHC + Prod. gain' = sf.large.plusgain,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.large
```

###Summary, Productivity models
Although we find significant effects of all productivity predictors, FHC emerges as the strongest (planned) predictor of successor knowledge in planned model comparisons.

#Math Facts model 
Is mean Math Facts performance significantly predictive of SF knowledge? Yes - Chisq(1) = 50.534, p < .0001
```{r}
#build model
...
##Model comparison, Math Facts + FHC
Does Math Facts explain unique variance on top of FHC? 
Yes - added to a model with FHC, mean math facts performance explains unique variance. 
```{r}
sf.large.plusmf <- glmer(Correct ~ mean.mf.c + fhc.c + count_range + age.c + (1|SID), 
                       family = "binomial", data = model.df)
anova(sf.large.base, sf.large.plusmf)

summary(sf.large.plusmf)
```


#Post-hoc analysis: Model comparison - Unit task, with HCNN
Although we preregistered the highest number for which a child could generate a successor, in future preregistrations we amended this measure to account for accuracy. Thus, in future work, we use Highest Contiguous Next Number - the highest number for which a child could generate a successor, given that the previous numbers were correct. This measure in the strongest predictor of Productivity (see above). Thus, this will be a follow-up analysis. 
```{r}
#base with hcnn
sf.hcnn.large.base <- glmer(Correct ~ highest_contig.c + count_range + age.c + (1|SID), 
                            family = "binomial", data = model.df)

##FHC+HCNN
sf.hcnn.large.plusfhc <- glmer(Correct ~ fhc.c + highest_contig.c + count_range + age.c + (1|SID), 
                            family = "binomial", data = model.df)

#comapre 
anova(sf.hcnn.large.base, sf.hcnn.large.plusfhc, test = 'LRT') #FHC significantly adds to this model

##IHC+FHC+HCNN
sf.hcnn.large.plusihc <- glmer(Correct ~ ihc.c + fhc.c + highest_contig.c + count_range + age.c + (1|SID), 
                            family = "binomial", data = model.df)
#compare
anova(sf.hcnn.large.plusfhc, sf.hcnn.large.plusihc, test = 'LRT') #IHC does not significantly add to this model 

##Productivity + FHC + HCNN
sf.hcnn.large.plusprod <- glmer(Correct ~ Productive + fhc.c + highest_contig.c + count_range + age.c + (1|SID), 
                            family = "binomial", data = model.df)
#compare
anova(sf.hcnn.large.plusfhc, sf.hcnn.large.plusprod, test = 'LRT') #Productivity does not add to this model

##Prod. gain
sf.hcnn.large.plusgain <- glmer(Correct ~ prod.gradient + fhc.c + highest_contig.c + count_range + age.c + (1|SID), 
                            family = "binomial", data = model.df)
#compare
anova(sf.hcnn.large.plusfhc, sf.hcnn.large.plusgain, test = 'LRT') #Prod. gradient does not add to this model

##Model table
mtable.sf.large.hcnn <- mtable('Base Model: HCNN' = sf.hcnn.large.base,
            'Model 1: FHC + HCNN' = sf.hcnn.large.plusfhc,
            'Model 2: IHC + FHC + HCNN' = sf.hcnn.large.plusihc,
            'Model 3: Productivity + FHC + HCNN' = sf.hcnn.large.plusprod,
            'Model 4: Prod. gain + FHC + HCNN' = sf.large.plusgain,
            summary.stats = c('Nagelkerke R-sq.','Log-likelihood','AIC','N'))
mtable.sf.large.hcnn
```

#Math Facts + HCNN + FHC
However, as we saw above, HCNN provides a better measure of productivity than FHC. Additionally, in other work, we have found that HCNN uniquely explains variance above and beyond FHC on the successor task. Thus, we are conducting a followup analysis using HCNN as a predictor. 

When included in a model with HCNN and FHC, MF is still a significant predictor of Unit Task performance. However, it is a less powerful one: because all measures are Z-scored, we can take the relative magnitude of the coefficients as indicators of their relative weighting in importance. 


```{r}
sf.large.plusmf <- glmer(Correct ~ mean.mf.c + fhc.c + highest_contig.c + count_range + age.c + (1|SID), 
                       family = "binomial", data = model.df)
anova(sf.hcnn.large.plusfhc, sf.large.plusmf)
...
#now pull out only the kids in the first quartile, plot performance on MF 
tmp %>%
  filter(Task == "SF")%>%
  group_by(SID, sf.quartile, mf.ceiling)%>%
  summarise(mean = mean(as.numeric(as.character(Correct), na.rm=TRUE)),
            sd = sd(as.numeric(as.character(Correct), na.rm=TRUE))) %>%
  ggplot(aes(x = mf.ceiling, y = mean, fill=mf.ceiling)) +
  stat_summary(fun.y = mean, position = position_dodge(width = .95), 
                      geom="bar", alpha = .5, colour = "black") +
  geom_violin(alpha = .1) +
  geom_point(aes(x = mf.ceiling, y = mean, colour = sf.quartile),
               position=position_jitter(width=0.15,height=0.02),
               size=1.5,
             alpha = .9,
               show.legend=TRUE, 
...
# #Can you predict whether someone will be productive or nonproductive based on their initial highest count?
#   
# model.df %<>%
