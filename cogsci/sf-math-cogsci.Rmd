...
On some accounts, [@carey2004; @carey2009], children's acquisition of this *Cardinal Principle* (CP, @gelman1978) is accomplished by an analogical mapping: children notice that both the count list and the cardinalities it represents both differ by exactly *one*, leading them to hypothesize that this may hold true for other numbers as well. Critically, such an account of CP-acqusition implicates the successor function.
...
<!-- Critically, the Peano axioms state that successor function holds for *all* natural numbers, meaning that the natural numbers are infinite -->


<!-- children's acquisition of this *Cardinal Principle* (CP) is driven by an analogical mapping made between the meanings of *one*, *two*, and *three* and the count list. <Such an account implicates the successor function><logical property which states that for any number there is a successor> -->
...
<!-- <!-- Although children encounter a multitude of challenges in acquiring symbolic number, one puzzle remains even after they have mastered the meaning of the count list and its relation to numerosity: linguistic number allows for expressions and representations of numerical infinity, yet early number experiences and input are decidedly finite. Nevertheless, by 5.5 -- 6 years of age, children generally understand that it is possible to add '1' to *any* number, and have even abstracted this principle to conclude that the natural numbers are endless [@cheung2017]. Somehow, children are able to go beyond their finite early number input to extract a set of principles which reveal the infinite nature of the natural numbers.  --> -->


<!-- <!-- How does such an understanding arise? What are the causal mechanisms underlying children's inference  --> -->



<!-- <!--  While this capacity makes us prodigious learners in general, one of its most powerful applications is in the domain of number. Although linguistic number input and expression are undoubtedly finite, we nevertheless come to know the natural numbers to be infinite; for every number, its successor can be  obtained by simply adding ‘1.’ Further, we understand that this successor function can be endlessly implemented due to the recursive nature of symbolic number. How does such an understanding arise? In this work, we explored two hypothesized causal mechanisms (productive counting knowledge and formally trained arithmetic) by which children might acquire the successor function, a critical component in the logical framework of natural number.  --> -->
